...
## p-value for rural urban difference in hh size
wilcox.test(hh_size ~ study_site, data = pa.pa)
...
## statistical test
pa.co.pa.age2 <- pa.co.pa.age%>%
  mutate(num_contacts = num_contacts/2)
pa.rural <- pa.co.pa.age%>%
  filter(study_site == "Rural")%>%
  mutate(num_contacts = num_contacts/2)%>%
  pull(num_contacts)
pa.urban <- pa.co.pa.age%>%
  filter(study_site == "Urban")%>%
  mutate(num_contacts = num_contacts/2)%>%
  pull(num_contacts)

ggplot(pa.co.pa.age2, aes(x = num_contacts, fill = study_site)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  facet_wrap(~study_site) +
  theme_minimal() +
  labs(title = "Distribution of num_contacts by Study Site")

shapiro.test(pa.rural)
shapiro.test(pa.urban)

## sample size is large enough (n > 30 for both)
## two-sample t-test

pa.svy <- svydesign(ids = ~1, weights = ~psweight, data = pa.co.pa.age2)
pa.ttest <- svyttest(num_contacts ~ study_site, design = pa.svy)
...
pa.co.we <- pa.co%>%
  left_join(pa.pa%>%select(rec_id, participant_age, participant_sex), by = "rec_id")%>%
  mutate(participant_age = case_when(participant_age == "<6mo" ~ "<5y",
                                     participant_age == "6-11mo" ~ "<5y",
                                     participant_age == "1-4y" ~ "<5y",
                                     TRUE ~ participant_age))%>%
...
##
pa.new.row <- data.frame(participant_age = c("<6mo", "6-11mo", "1-4y", "<6mo", "6-11mo", "1-4y"), 
                         study_site = c("Rural", "Rural", "Rural", "Urban", "Urban", "Urban"),
                         pop = NA, prop = NA, n_s = NA, prop_s = NA,
                         psweight = c(0.4527577, 0.4527577, 0.4527577, 0.4326847, 0.4326847, 0.4326847))
pa.we.mod <- pa.we%>%
  rbind(pa.new.row)%>%
  filter(participant_age != "<5y")
pa.co.we2 <- pa.co%>%
  left_join(pa.pa%>%select(rec_id, participant_age), by = "rec_id")%>%
  left_join(pa.we.mod%>%select(psweight, participant_age, study_site), by = c("participant_age", "study_site"))%>%
...
pa.co.pa.counts %>%
  ...
  left_join(o.denoms.byage.pa, by = "participant_age") %>%
  mutate(mean_conthours = cont_time / n)-> mean_cont_time_pa
